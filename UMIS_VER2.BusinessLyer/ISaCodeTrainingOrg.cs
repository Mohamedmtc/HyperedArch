using System;
using System.ComponentModel;

namespace UMIS_VER2.BusinessLyer
{
	/// <summary>
	///		The data structure representation of the 'SA_CODE_TRAINING_ORG' table via interface.
	/// </summary>
	/// <remarks>
	/// 	This struct is generated by a tool and should never be modified.
	/// </remarks>
	public interface ISaCodeTrainingOrg 
	{
		/// <summary>			
		/// SA_CODE_TRAINING_ORG_ID : 
		/// </summary>
		/// <remarks>Member of the primary key of the underlying table "SA_CODE_TRAINING_ORG"</remarks>
		System.Decimal SaCodeTrainingOrgId { get; set; }
				
		/// <summary>
		/// keep a copy of the original so it can be used for editable primary keys.
		/// </summary>
		System.Decimal OriginalSaCodeTrainingOrgId { get; set; }
			
		
		
		/// <summary>
		/// TRAININRG_CODE : 
		/// </summary>
		System.String  TraininrgCode  { get; set; }
		
		/// <summary>
		/// TRAININRG_DESCR : 
		/// </summary>
		System.String  TraininrgDescr  { get; set; }
		
		/// <summary>
		/// TRAININRG_ADDRESS : 
		/// </summary>
		System.String  TraininrgAddress  { get; set; }
		
		/// <summary>
		/// TRAININRG_CONTACT_PERSON : 
		/// </summary>
		System.String  TraininrgContactPerson  { get; set; }
		
		/// <summary>
		/// GS_COUNTRY_NODE_ID : 
		/// </summary>
		System.Decimal  GsCountryNodeId  { get; set; }
		
		/// <summary>
		/// GS_CONTACT_METHOD_H_ID : 
		/// </summary>
		System.Decimal  GsContactMethodHId  { get; set; }
			
		/// <summary>
		/// Creates a new object that is a copy of the current instance.
		/// </summary>
		/// <returns>A new object that is a copy of this instance.</returns>
		System.Object Clone();
		
		#region Data Properties


		/// <summary>
		///	Holds a collection of entity objects
		///	which are related to this object through the relation _saTrainingCourseSaCodeTrainingOrgId
		/// </summary>	
		TList<SaTrainingCourse> SaTrainingCourseCollection {  get;  set;}	

		#endregion Data Properties

	}
}


